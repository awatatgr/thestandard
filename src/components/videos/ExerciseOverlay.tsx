...
export function ExerciseOverlay({ exercises, currentTime, onSeek }: ExerciseOverlayProps) {
  ...
  return (
    <div className="absolute inset-0 pointer-events-none z-10">
      ...
      <div className="absolute bottom-3 left-0 right-0 flex justify-center items-end gap-0.5 sm:gap-1 px-3 pointer-events-auto">
        {exercises.map((ex, i) => {
          ...
          return (
            <button
              key={i}
              type="button"
              className={`flex flex-col items-center gap-0.5 ${onSeek ? "cursor-pointer" : "cursor-default"}`}
              onClick={onSeek ? () => onSeek(ex.startSeconds) : undefined}
              title={ex.name}
            >
              {isActive && (
